...
from src.db.db_auth import conn, psycopg2
...
def insert_branches(branches):
    ...
    for i in range(rows_len):
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"SELECT country_id FROM countries WHERE country_name='{country}'")
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"SELECT region_id FROM regions WHERE country_id={country_id} AND region_name='{region}'")
...
def insert_employees(employees):
    ...
    for i in range(rows_len):
        ...
        name_and_patronymic = employee['name'][i] + \
            ' ' + employee['patronymic'][i]
